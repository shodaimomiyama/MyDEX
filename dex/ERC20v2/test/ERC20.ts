...
import { loadFixture } from "@nomicfoundation/hardhat-network-helpers"; //deploy条件(Fixture)をまとめて、まとめたものを呼び出せる。

async function deployContractsFixture() {
    const [account0, account1, account2] = await ethers.getSigners();
    const ERC20 = await ethers.getContractFactory("ERC20", account0);
    const erc20 = await ERC20.deploy("Efits", "EFT", 18);
...
describe("ERC20 contract states", function () {
    it("getters", async function () {
        ...
        expect(await erc20.name()).to.equal("Efits");
        expect(await erc20.symbol()).to.equal("EFT");
...
describe("ERC20 mint", function () {
    it("mint", async function () {
        ...
        const amount1: bigint = 10n * (10n ** BigInt(decimals)); // 10Efits decimals=3 0.001 => 1, 0.001 * (10 ** 3)
        const tx1 = await erc20.mint(account1.address, amount1); //txの送信が完了した(実行はまだ)
        const receipt1 = await tx1.wait(); //txの実行が完了した
    ...
    it("mint from non-owner", async function () { //owner(msg.senderの自分)以外のアカウント(account1)がERC20コントラクトと勝手にconnectし、balanceをamount分増やすことができないことをテスト
        const { erc20, account0, account1, account2 } = await loadFixture(deployContractsFixture);
        const amount = 123;
    ...
    it("mint overflow", async function () {
        ...
        erc20.mint(account1.address, uint256Max);
...
describe("ERC20 transfer", function () {
    it("transfer and Transfer event", async function () {
        ...
        // console.log(typeof erc20.transfer);
        await expect(erc20.connect(account1).transfer(account2.address, amount)).to.emit(erc20, "Transfer").withArgs(account1.address, account2.address, amount); //withArgs：どんな引数を得たか
