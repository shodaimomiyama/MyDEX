...
contract UdexRouter {
    address public immutable factory; //immutable:初期化フェーズに任意の値を不変値として定義すること

    //factory変数はimmutableなので、一度constructorで値が決まると
    ...
    //amountAとamountBの比率を流動性プールにあるtokenAとtokenBの比率に合わせる。
    function _addLiquidity(
        ...
        uint reserve0 = UdexPool(pool).reserve0(); //流動性プール内のリザーブの量を取得
        ...
        //流動性プール内が空の時→希望の預入額をそのままいれればOK
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = UdexLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            ); //Aの希望額から計算されるBの量
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "UdexRouter: INSUFFICIENT_B_AMOUNT"
                );
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = UdexLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
                ...
                require(
                    ...
                    "UdexRouter: INSUFFICIENT_A_AMOUNT"
                );
    ...
    function addLiquidity(
        ...
        uint amountBMin,
        address to,
        uint deadline //tx実行の期限
    )
        external
        ensure(deadline)
        returns (uint amountA, uint amountB, uint liquidity)
    {
        address pool = UdexFactory(factory).getPool(tokenA, tokenB); //token addressを渡してpool addressを取得
        //流動性プールが存在しない場合pool address==address(0)
        if (pool == address(0)) {
            UdexFactory(factory).createPool(tokenA, tokenB);
            pool = UdexFactory(factory).getPool(tokenA, tokenB);
        }
        //預け入れるtokenAとtokenBを計算
        ...
        //tokenA,tokenBを入金
        //User ⇔ RouterContract ⇔ TokenContract　の位置関係。だからtransFerfrom(User == msg.sender)
        ...
        //transferFromとmintを同じトランザクションで行うことによりセキュアになる。（流動性トークン横取りなどがなくなる）
    }

    //流動性トークンをバーンしてトークンペアを取り出す。
    function removeLiquidity(
        ...
        (uint amount0, uint amount1) = UdexPool(pool).burn(to); //戻り値はburnによって得られたトークンの量
        (amountA, amountB) = tokenA < tokenB
            ? (amount0, amount1)
            : (amount1, amount0); //tokenaddressの順序に合わせてamountAとamountBに書き換える。
        require(amountA >= amountAMin, "UdexRouter: INSUFFICIENT_A_AMOUNT");
        require(amountB >= amountBMin, "UdexRouter: INSUFFICIENT_B_AMOUNT");
    }

    //dexのuserがトークンを交換する時に呼び出す関数
    function swapTokenPair(
...

        {
            ...
            (uint reserveIn, uint reserveOut) = tokenIn < tokenOut
                ? (reserve0, reserve1)
                : (reserve1, reserve0);
            amountOut = UdexLibrary.getAmountOut(
                amountIn,
                reserveIn,
                reserveOut
            );
        ...
        require(
            ...
            "UdexRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
