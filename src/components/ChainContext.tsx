...
export const ChainContextProvider = ({ children }: ChainContextProviderProps) => {
    const [chainId, setChainId] = useState<number>() //ブロックチェーンを指定
    const [currentAccount, setCurrentAccount] = useState<string>() //WalletAccount
    const [signer, setSigner] = useState<Signer>()
    const [factoryAddress, setFactoryAddress] = useState<string>()
    const [routerAddress, setRouterAddress] = useState<string>()

    async function connectWallet() {
        const provider = await detectEthereumProvider({ silent: true }) //silent: providerに値が入らないときに、エラーにならずnullになる
        if (provider) {
